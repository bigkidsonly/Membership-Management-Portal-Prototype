...
From connection through collaboration, Tableau is the most powerful, secure, and flexible end-to-end analytics platform for your data. Elevate people with the power of data. Designed for the individual, but scaled for the enterprise, Tableau is the only business intelligence platform that turns your data into insights that drive action.`,
    features: ['Interactive dashboards with drag-and-drop functionality', 'Real-time data visualization and analytics', 'Connect to virtually any data source', 'Mobile-friendly design for on-the-go access', 'Enterprise-grade security and governance', 'Powerful AI-driven analytics with Ask Data and Explain Data', 'Seamless integration with Salesforce CRM'],
    screenshots: ['https://help.tableau.com/current/pro/desktop/en-us/Img/sample_your_data.png', 'https://help.tableau.com/current/pro/desktop/en-us/Img/viz_types_all.png'],
    pricingTiers: [{
      name: 'Creator',
      regularPrice: 70,
      memberPrice: 49,
      features: ['Full authoring capabilities', 'Connect to all data sources', 'Web and desktop access']
    }, {
      name: 'Explorer',
      regularPrice: 42,
      memberPrice: 29.4,
      features: ['Web authoring capabilities', 'Connect to published data sources', 'Save and share work']
    }, {
      name: 'Viewer',
      regularPrice: 15,
      memberPrice: 10.5,
      features: ['View and interact with dashboards', 'Subscribe to content updates', 'Mobile access']
    }],
    reviews: [{
      author: 'Digital Democracy Project',
      rating: 5,
      date: 'March 10, 2023',
      comment: 'Tableau has transformed how we analyze and present voter engagement data. The cooperative discount made it accessible for our entire team.'
    }, {
      author: 'Tech Justice Collective',
      rating: 4,
      date: 'February 22, 2023',
      comment: 'Great visualization capabilities. The learning curve is a bit steep, but the TMC onboarding support was extremely helpful.'
    }, {
      author: 'Community Tech Hub',
      rating: 5,
      date: 'January 15, 2023',
      comment: 'We use Tableau for all our community impact reporting. The member discount and additional user licenses have been game-changing for our small nonprofit.'
    }],
    memberBenefits: ['Enhanced support from Salesforce', 'Priority onboarding assistance', '3 additional user licenses', 'Custom integration support', 'Access to Tableau Conference at discounted rates']
  };
  // Similar tools
  const relatedTools = [{
    id: '4',
    ...
    category: 'Data & Analytics',
    vendor: 'Microsoft',
    description: 'Business analytics service that delivers insights for analyzing data. Share insights across your organization.',
    regularPrice: 9.99,
    memberPrice: 6.99,
    discount: 30,
    tier: 'TL-1',
    ...
    category: 'Data & Analytics',
    vendor: 'Google',
    description: 'Business intelligence software and big data analytics platform that helps you explore, analyze and share business insights.',
    regularPrice: 50,
    memberPrice: 35,
    discount: 30,
    tier: 'TL-1',
  ...
  return <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden mb-8">
        <div className="p-6 border-b border-gray-200">
          <div className="flex flex-col md:flex-row justify-between">
            <div className="flex items-center mb-4 md:mb-0">
              <img src={tool.logo} alt={`${tool.vendor} logo`} className="h-16 mr-4 object-contain" />
              <div>
            ...
            <div className="flex flex-col items-end">
              ...
              <span className={`px-2 py-1 text-xs font-medium rounded-full mb-3 ${tool.tier === 'TL-1' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
                {tool.tier} Access
              </span>
              <Link to={`/marketplace/purchase/${tool.id}`} className="px-6 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors">
                Request Purchase
              </Link>
            </div>
          </div>
        ...
        <div className="border-b border-gray-200">
          <nav className="flex -mb-px">
            {['overview', 'pricing', 'reviews', 'implementation', 'support'].map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`py-4 px-6 text-sm font-medium border-b-2 ${activeTab === tab ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
        ...
        <div className="p-6">
          {activeTab === 'overview' && <div>
              ...
              <ul className="space-y-2 mb-6">
                {tool.features.map((feature, index) => <li key={index} className="flex items-start">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                {tool.screenshots.map((screenshot, index) => <img key={index} src={screenshot} alt={`${tool.name} screenshot ${index + 1}`} className="rounded-lg shadow-sm w-full h-auto" />)}
              </div>
            </div>}
          {activeTab === 'pricing' && <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                {tool.pricingTiers.map((tier, index) => <div key={index} className="border rounded-xl p-6 hover:shadow-md transition-shadow">
          ...
          {activeTab === 'reviews' && <div>
              ...
              <div className="space-y-6 mb-6">
                {tool.reviews.map((review, index) => <div key={index} className="border-b border-gray-200 pb-6 last:border-0">
      ...
      <div className="bg-blue-50 rounded-xl shadow-sm p-6 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {tool.memberBenefits.map((benefit, index) => <div key={index} className="flex items-start">
              <Check className="h-5 w-5 text-blue-600 mr-2 flex-shrink-0 mt-0.5" />
