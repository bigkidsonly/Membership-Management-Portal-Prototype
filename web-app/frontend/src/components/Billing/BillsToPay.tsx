...
export function BillsToPay({ invoices, onPayNow }: BillsToPayProps) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 border border-gray-100 transform transition-all duration-300 hover:shadow-xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {billCategories.map((category, index) => (
          <div key={index} className="space-y-3">
            <div
              className={`rounded-lg p-5 border ${category.borderColor} ${
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  ...
                  <span className="text-sm font-medium text-gray-700">
                    ...
                    {index === 0 && category.count > 0 && (
                      <span className="ml-2 inline-flex items-center justify-center bg-red-100 text-red-600 w-5 h-5 rounded-full text-xs font-bold">
                        {category.count}
                      </span>
                    )}
                  </span>
                ...
                {category.invoices.length > 0 && (
                  <button
                    ...
                    className="p-1 rounded-full hover:bg-black text-gray-500"
                  >
              ...
              {category.amount > 0 && (
                <div className="mt-3 flex justify-between items-center">
                  ...
                  <button
                    onClick={() => onPayNow && onPayNow(category.invoices[0])}
                    className={`text-xs flex items-center font-medium px-3 py-1.5 rounded-lg ${category.buttonColor} shadow-sm hover:shadow transition-all duration-200`}
                  >
            ...
            {expandedCategory === index && category.invoices.length > 0 && (
              <div className="bg-gray-50 rounded-lg border border-gray-200 p-3 shadow-sm space-y-2 text-sm">
      ...
      {overdueAmount > 0 && (
        <div className="mt-5 bg-red-50 p-4 rounded-lg border border-red-100 text-sm text-red-600 flex items-start">
      ...
      {totalToPay > 0 && (
        <div className="mt-5 flex justify-end">
          <button
            ...
            className="px-5 py-2.5 bg-primary text-white rounded-lg hover:bg-primary/90 transition-all duration-300 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 flex items-center font-medium"
          >
