...
import { Clock, CheckCircle, AlertTriangle, XCircle, DollarSign, Calendar, User } from 'lucide-react';
...
export function BillingHeader({
  ...
  const renderStatusBadge = () => {
    switch (membershipData.status) {
      case 'active':
        return <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 shadow-md">
            <CheckCircle className="w-3 h-3 mr-1.5" />
            Active
          </span>;
      case 'expiring':
        return <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 shadow-md">
            <AlertTriangle className="w-3 h-3 mr-1.5" />
            Expiring Soon
          </span>;
      case 'expired':
        return <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 shadow-md">
  ...
  return <div className="bg-white rounded-xl shadow-lg p-8 border border-gray-100 transform transition-all duration-300 hover:shadow-xl relative overflow-hidden">
      ...
      <div className="relative flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          <div className="flex items-center">
            <div className="bg-gradient-to-br from-primary to-primary/80 p-3 rounded-lg mr-3 shadow-md">
